 ...
 * {@link uk.ac.aber.dcs.cs12420.aberpizza.data.Side} item to the current order.
 * 
...
public class SidePanel extends JPanel implements ActionListener {
	...
	/** The side size and type combo boxes. */
	private JComboBox sizeCombo, typeCombo;
	
	/** The button to add the selected side to the order */
	private JButton addSide;
	
	/** The number model for the quantity spinner component. */
	private SpinnerNumberModel quantModel;
	
	/** The spinner to select the quantity of the side. */
	...
	/**
	 * Instantiates a new Side panel. Creates 'link' objects to {@link uk.ac.aber.dcs.cs12420.aberpizza.data.Till} data object
	 ...
	 */
	public SidePanel(Till tillObject, DisplayBasePanel displayPan) {
		
		//Set the size of the panel
		this.setPreferredSize(new Dimension(360, 130));
		
		//Create a titled border around the panel
		this.setBorder(BorderFactory.createTitledBorder("Sides"));
	...
	public void initComponents() {
		...
		//Create new instances of JComboBox and populate with Size and SideType enum values
		sizeCombo = new JComboBox(Size.values());
		typeCombo = new JComboBox(SideType.values());
		...
		addSide = new JButton("Add Side to Order");
	...
	public void setUpLayout() {
		...
		layout.putConstraint(SpringLayout.WEST,sizeLabel,25,SpringLayout.WEST,this);
		
		layout.putConstraint(SpringLayout.NORTH,sizeCombo,10,SpringLayout.SOUTH,sizeLabel);
		layout.putConstraint(SpringLayout.WEST,sizeCombo,25,SpringLayout.WEST,this);
		...
		layout.putConstraint(SpringLayout.NORTH,addSide,10,SpringLayout.SOUTH,sizeCombo);
		layout.putConstraint(SpringLayout.WEST,addSide,0,SpringLayout.EAST,sizeCombo);
	}
	...
	public void actionPerformed(ActionEvent evt) {
		...
		//If the "Add Side to Order" button is clicked
		if (actionCommand.equals("Add Side to Order")) {
			try {
				
				/* Create a new Side object inside new order using selected options from combo boxes and spinner,
				 * before adding to the array list of items inside order.
				 */
				tillObject.getNewOrder().createNewSide((Size)sizeCombo.getSelectedItem(), (SideType)typeCombo.getSelectedItem(), Integer.parseInt((String) quantSpinner.getValue().toString()));
